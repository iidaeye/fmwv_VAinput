...
| 項目 | 状態 |
|---|---|
| **P1 token プール化** | ✅ **完了・維持**（既存 self-checkin ops/session = 535、新クライアント `<IP-C>` も同基盤で 2,181） |
| **P4 ClinicReceptionStatus 400 バグ** | ✅ **完了・維持**（400 ゼロ継続、GET パスのみ） |
| **P2 `cPt_VisitRecord/_find` 縮小** | 🔴 **現在の最優先**（self-checkin 総レスポンスの >99%、2.44 GB/h）。**読み書き分離レイアウト**（read-slim / write-lean）で実装予定 |
| P5 ClinicScheduleException キャッシュ | 🟡 latest で 22,310 件/89分 全 0 件 → 次点で着手価値あり |
| P3 / P6 / P7 | 未着手 |
...
| post-fix | 2026-05-16 09:23–22:15（約13h） | P1/P4 稼働後の初回確認 |
| **latest** | **2026-05-22 12:06–13:35（89分・日中繁忙）** | **P1/P4 稼働 1 週後の追跡。新クライアント `<IP-C>` が同パターンで合流** |

### self-checkin（`<IP-A>` / `<IP-C>`）構造指標

| 指標 | baseline | post-fix(05-16) | latest(05-22) | 判定 |
|---|---|---|---|---|
| **ops/session**（`<IP-A>`） | **1.00** | 163（12–14時は 376–425） | **535** | ✅ P1 維持＋改善 |
| **ops/session**（新 `<IP-C>`） | — | （test 36件のみ） | **2,181**（sign-in 7 / op 15,266） | ✅ 新クライアントもプール化済 |
| sign-in 回数（self-checkin 合算） | ~44,000 /時 | ~150 /時 | **~107 /時** | ✅ 約 99.76% 削減 |
| sign-out（DELETE）数 | 73,848 /12h | 2,462 /13h | 297 /89分 | ✅ |
| `ClinicReceptionStatus/_find` の 400 | 100%（2,707–2,917件） | 0 件 | **0 件**（維持） | ✅ P4 維持 |
| 〃 置換先 GET `…/records` 件数 | — | 28,423（全 OK） | 13,634（全 OK） | ✅ |

### `cPt_VisitRecord/_find`（成功レスポンス）サイズ推移

| | baseline(昼) | overnight(夜) | post-fix(05-16) | **latest(05-22)** |
|---|---|---|---|---|
| p50 | 35 KB | 230 KB（一時悪化） | 62 KB | **88 KB** |
| mean | 171 KB | 252 KB | 151 KB | **178 KB** |
| 累計 | 2.6 GB/79分 | — | 7.7 GB/13h | **3.6 GB/89分 ≒ 2.44 GB/h** |

→ overnight の肥大化はデータ依存（履歴の多い患者）による変動だった可能性大。
latest では mean がやや増（151→178 KB）、p50 も増（62→88 KB）。
**P1 でスループットが上がった結果、重いクエリが詰まりやすい時間帯がある**可能性。

**結論**: token/400 解消後は本 EP が単独で self-checkin 総レスポンスの **>99%** を占有。
P2（読み書き分離レイアウト: read-slim / write-lean）が**現状唯一の高効果策**。

### 副次観察（latest）

- **`ClinicScheduleException/_find[err=401]`**: self-checkin 17,256 + `<IP-C>` 4,681 + LIFF 373 = **22,310 件 / 89分が全 0 件ヒット**
  - LIFF では全 ops の **32.6%** を占有 → **P5 のクライアントキャッシュ 5 分**で大幅削減見込み
- 401（"no records"）全体で 40,628 件（全リクエストの 39.5%）
- LIFF (`<IP-B>`) は ops/session = 4.13、構造変化なし（P3 未着手）
...
### 🔴 P2: `cPt_VisitRecord/_find` の読み書き分離レイアウト化 — **現在の最優先**

**現状（latest 2026-05-22）**: P1/P4 解消後、本 EP が self-checkin 総レスポンスの **>99%** を単独占有。
latest で **21,343 件 × mean 178 KB ≒ 3.6 GB / 89 分（= 2.44 GB/h）**（p50 88 KB）。
token/セッション・オーバーヘッドが消えた今、**体感速度の支配項はこの 1 本**。

**対応方針: 読み書きでレイアウトを分離する**

FileMaker Data API は **叩いたレイアウト上の全フィールド＋ポータルをレスポンスにシリアライズ**するため、
読み専用と書き専用でレイアウトを分けると **2 重に効く**：

| 用途 | 新レイアウト案 | 中身 | 効く理由 |
|---|---|---|---|
| 読み（`_find`） | `api_cPt_VisitRecord_read` | 表示・判定に必要なフィールドのみ。**ポータル無し**。非保存計算は最小化 | レスポンス JSON が小さくなる、`_find` 時のレイアウト評価が軽くなる |
| 書き（`PATCH /records/{id}`） | `api_cPt_VisitRecord_write` | 更新する数フィールドのみ。**ポータル無し**、**非保存計算無し** | PATCH commit 時に重い非保存計算・ポータル評価を踏まない |

**着手手順**:
1. 現在の `api_cPt_VisitRecord` レイアウトのフィールド数 / ポータル / 非保存計算 / `limit` の現状を把握
2. self-checkin / LIFF それぞれが**読みで実使用しているフィールド一覧**を抽出（クライアントコード or レスポンス利用箇所から）
3. 上記 2 レイアウトを作成し、self-checkin・LIFF を**読み/書きで叩き分け**
4. `_find` 側は **`limit` 明示**（1件用途で多件返していないか確認）、必要ならポータル行を絞った版を別 EP に
5. （余地）FM 22 の **OData `$select`** による列指定読みも別途 PoC

**期待効果**: mean 178 KB → 数 KB〜十数 KB 級（必要フィールドだけに依存）。
帯域・FM レイアウト評価・JSON 化が **3〜10 倍**改善見込み。書き込み側も commit が軽くなる。

**トレードオフ**:
- FM スキーマ上の保守対象が 1 → 2 レイアウトに増える
- クライアント（self-checkin・LIFF）双方で**読み/書きの呼び出し先を出し分ける**改修が必要

**検証**: `cluster-transactions.mjs` の `cPt_VisitRecord/_find` 行 "mean KB" が劇的に下がること。
PATCH 側は応答サイズが元々小さいので、効果は **書き込み所要時間**で観測（必要なら `measure.mjs` で計測）。

> 注: overnight ログで p50 が一時 230 KB に肥大したが post-fix で 62 KB、latest で 88 KB。
> これはデータ依存（履歴の多い患者）の変動であり、read-slim 化すれば
> この変動自体も縮小する。
